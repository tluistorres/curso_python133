...
# 1º pai, 2º pasta a ser adicionada.

print(caminho_completo)    # C:\'projetos_python'
os.mkdir(caminho_completo) 

# os.mkdir(): é uma função do módulo 'os' que cria um novo diretório.

os.listdir()               # Verifica onde o diretório foi criado.

# Renomear uma pasta.

os.rename('C:\\workspace', 'C:\\projetos_py')  

# python renomear_diretorio.py
...
# Verifica o nome do arquivo a partir do comando completo. 'Teste'

os.path.dirname(os.getcwd())   

# Para saber onde está o diretótio ou arquivo. 'C:\\'
...
# C:\Users\luist\OneDrive\Documents\workspace\América\Brasil\Alagoas

os.makedirs(caminho)         

# Cria as pastas e verifica no Meu Computador as pastas criadas.

os.path.exists('C:workspace\\América')   # Verifica se o caminho existe.

# Verificação.

os.path.isfile('C:Teste') 

# True ou False  # Existe a pasta 'Teste' ? Path quer dizer caminho.
...
arquivo = os.path.basename('C\\workspace\\arq.txt') 

# Como você gravou o arquivo ?
...
# Resposts: ('arq', '.txt') - Divide o nome de sua extenão. Muito útil, muito mesmo. Podemos excluir, criar, renomear os arquivos em massa, utilizando s=este tipo de técnica(splitext). Arwquivos imagens.
...
# windowsPath('C:/workepace') Mostra que estamos nowindows.
...
import shutil
shutil.rmtree('C:\\Teste')  

# Cuidado pra não remover pastas que não queira.
...
# Vamos criar uma pasta no explorer com vários arquivos para seree nomeados. Podemos também criar o arquivo 'Teste' no vScode seguindo os passos anteriores.
...
for contador,arq in enumerate(os.listdir()): 
    ...
    # fazer a numeração com um ídice.

    if os.path.isfile(arq):                 
        
        # Se for arquivos ? Numerá-los.

        nome_arq, exten_arq = os.path.splitext(arq)
        
        # Separar nome da extensão, usando o splitext.

        nome_arq = padrão_nome + '' + str(contador+1) # Poderis ser: contador + 1 ?
        nome_novo = f'{nome_arq}{exten_arq}'
        os.rename(arq, nome_novo)    
        
        # nome-novo será digitado pelo usuário. Sug. Dados.

print(f'\nArquivos  renomeados.')    

# Próximo passo é rodar o script.
